...
if (mysqli_connect_error()) {
  $logMessage = 'gameSnap: MySQL Connect Error: ' . mysqli_connect_error();
...
if ($result1) {
  ...
  error_log("gameSnap: Check for valid snapshot ID failed: " . $cpid);
  $_SESSION['SESS_HEADER_MESSAGE'] = 
    'The selected snapshot is not in the data base!';
  header("location: board18Main.php");
  }
} else {
  error_log("gameSnap: Check for valid snapshot ID: Query failed");
  exit;
}
