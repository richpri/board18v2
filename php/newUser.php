...
if ( !$link ) {        
  error_log('newUser.php: mysql connect error: ' . mysqli_connect_error());
  echo 'fail';
  exit; 
}
...
if($result1) {
  ...
  error_log("newUser.php: Check duplicate name: Query failed");
  echo 'fail';
  exit;
}
...
if($result2) {
  ...
  error_log("newUser.php: Check duplicate email: Query failed");
  echo 'fail';
  exit;
}
...
if($result3) {
  ...
  error_log("newUser.php: Check first player: Query failed");
  echo 'fail';
  exit;
}
...
if($result) {   // Was the query successful
  ...
  error_log("newUser.php: Insert new player: Query failed");
  echo 'fail';
}
