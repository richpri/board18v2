/*
 * board18Misc2.js contains all of the code to actually process
 * the buttons on game session edit form and to inform players 
 * of any change in which games they are playing.
 * 
 ...
 * click method of the submit button on the
 * game session edit form. 
...
function changePlayer(login, game) {
  ...
  if (prem !== "") {  // Has a "remove player" login been entered?
    if (prem === login) { // Is player trying to remove himself??
      if (BD18.self === 0) { // Is this the first try?
        BD18.errtxt = 'Press submit again to remove yourself.';
        $("#pname3_error").text(BD18.errtxt).show(); // Ask if he is sure.
    ...
    var gameplayer = [];
    BD18.gamePlayers.forEach(makeGamePlayerArray);
    function makeGamePlayerArray(item, index) {
       gameplayer[index] = item["login"];
    }
    if (!gameplayer.includes(prem)) {
      BD18.errtxt = 'Player ' + prem;
      BD18.errtxt += ' does not exist in the game.';
      $("#pname3_error").text(BD18.errtxt).show();  
      $("#pname3") .trigger('focus');
      return false;
    }
    BD18.self = 0;
  ...
  if (padd !== "") { // Has an "add player" login been entered?
    BD18.playerList.forEach(makeAllPlayerArray);
    function makeAllPlayerArray(item, index) {
       allplayer[index] = item["login"];
    }
    if (!allplayer.includes(padd)) {
      BD18.errtxt = 'Player ' + padd;
      BD18.errtxt += ' does not exist in the database.';
      $("#pname4_error").text(BD18.errtxt).show();  
    ...
    BD18.errtxt = 'No player action selected.';
    $("#pname4_error").text(BD18.errtxt).show();
    $("#pname4") .trigger('focus');
    return false;
  }
 ...
 *
 * The response from linkGet.php is the following stringified 
 * JSON data structure. 
 * 
 *   {
...
function addLink() {
  ...
  BD18.newlink = lname1;
  var format = /[!@#$%^&*()+\=\[\]{};':"\\|,<>\/?]+/;
  if(format.test(BD18.newlink)){ 
    $("#lname1_error").text('Link name cannot contain special characters.').show();
    $("#lname1") .trigger('focus');  
    return; 
  }
  var ascii = /^[\x00-\x7F]*$/;
  if(!ascii.test(BD18.newlink)){
    $("#lname1_error").text('Link name can only contain ascii characters.').show();
    $("#lname1") .trigger('focus');  
    return; 
  }
  if(BD18.newlink.length > 25){
    $("#lname1_error").text('Link name must be 25 characters or less.').show();
    $("#lname1") .trigger('focus');  
    return; 
  }
